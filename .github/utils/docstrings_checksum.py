...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", help="Deepstack root folder", required=True, type=Path)
    args = parser.parse_args()

    # Get all Deepstack and rest_api python files
    root: Path = args.root.absolute()
    deepstack_files = root.glob("deepstack/**/*.py")
    rest_api_files = root.glob("rest_api/**/*.py")

    import itertools

    python_files = itertools.chain(deepstack_files, rest_api_files)
