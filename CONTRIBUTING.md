...
We are very open to community contributions and appreciate anything that improves `deepstack`! This includes fixings typos, adding missing documentation, fixing bugs or adding new features.
To avoid unnecessary work on either side, please stick to the following process:

1. Check if there is already [a related issue](https://github.com/khulnasoft/deepstack/issues).
2. If not, open a new issue to start a quick discussion. Some features might be a nice idea, but don't fit in the scope of Deepstack and we hate to close finished PRs!
...
Even though Deepstack runs on Linux, MacOS and Windows, we current we mostly support development on Linux and MacOS. Windows contributors might encounter issues. To work around these, consider using [WSL](https://docs.microsoft.com/en-us/windows/wsl/about) for contributing to Deepstack.
...
```bash
git clone https://github.com/<your-gh-username>/deepstack.git
```
...
# Install Deepstack in editable mode
pip install -e '.[dev]'
```
...
This utility will run some tasks right before all `git commit` operations. From now on, your `git commit` output for Deepstack should look something like this:
...
Most of the changes to Deepstack, including bug fixes and small improvements, are implemented through the normal Pull Request workflow, according to these  contribution guidelines.
...
### PR Description
Please use the existing [pull request template](https://github.com/khulnasoft/deepstack/blob/main/.github/pull_request_template.md)
...
impact on Deepstack behavior, for example: tests and documentation updates, changes in code comments or docstrings,
...
enhancements:
  - |
    Upgrade transformers to the latest version 4.31.0 so that Deepstack can support the new LLama2 models.
```
...
# Weaviate
docker run -d -p 8080:8080 --name deepstack_test_weaviate --env AUTHENTICATION_ANONYMOUS_ACCESS_ENABLED='true' --env PERSISTENCE_DATA_PATH='/var/lib/weaviate' --env ENABLE_EXPERIMENTAL_BM25='true' semitechnologies/weaviate:1.14.1
...
We formally define three scopes for tests in Deepstack with different requirements and purposes:
...
- **Goal: being confident in releasing Deepstack**
...
def test_elasticsearch_custom_fields(elasticsearch_fixture):
    ...
    client.indices.delete(index='deepstack_test_custom', ignore=[404])
    document_store = ElasticsearchDocumentStore(index="deepstack_test_custom", text_field="custom_text_field",
...
You can run it with `python -m mypy deepstack/ rest_api/` from the root folder.

### Pylint
Pylint is still being integrated in Deepstack. The current exclusion list is very long, and can be found in `pyproject.toml`.

You can run it with `python -m pylint deepstack/ rest_api/ -ry` from the root folder.


## Contributor Licence Agreement (CLA)

Significant contributions to Deepstack require a Contributor License Agreement (CLA). If the contribution requires a CLA, we will get in contact with you. CLAs are quite common among company backed open-source frameworks and our CLA’s wording is similar to other popular projects, like [Rasa](https://cla-assistant.io/RasaHQ/rasa) or [Google's Tensorflow](https://cla.developers.google.com/clas/new?domain=DOMAIN_GOOGLE&kind=KIND_INDIVIDUAL) (retrieved 4th November 2021).

The agreement's main purpose is to protect the continued open use of Deepstack. At the same time it also helps in protecting you as a contributor. Contributions under this agreement will ensure that your code will continue to be open to everyone in the future (“You hereby grant to Khulnasoft **and anyone** [...]”) as well as removing liabilities on your end (“you provide your Contributions on an AS IS basis, without warranties or conditions of any kind [...]”). You can find the Contributor Licence Agreement [here](https://cla-assistant.io/khulnasoft/deepstack).

If you have further questions about the licensing feel free to reach out to contributors@khulnasoft.com.
