...
- Proposal PR: https://github.com/khulnasoft/deepstack/pull/4084
...
Current Deepstack pipeline design approach fits nicely with these extension modules/tools. They can be tested in
isolation and subsequently integrated into the agent. The agent can register these tools, understand their objectives
and route the user input to the appropriate tool. The tools can be either Deepstack pipelines or components.


A demo idea for the new Agent is to demonstrate Question Answering on Technical Documentation (using the example of
Deepstack). It should be useful and impressive, and it determines what tools we will implement first.

**Example Questions:**
- "Why am I seeing duplicate answers being returned?" based on indexed FAQ documentation
- "Which organizations use Deepstack?" based on web search and Wikipedia returning answers via SearchEngine
...
demonstrating how to use an agent with tools and a [branch](https://github.com/khulnasoft/deepstack/compare/main...mrkl-pipeline)
...
we can search through Deepstack documentation on docs.deepstack.com, github.com/khulnasoft/deepstack and so on.
...
SearchEngine that produces a list of Deepstack Documents.
...
in Deepstack can use it as well.
...
## khulnasoft Cloud API
The Agent should be able to use pipelines deployed on khulnasoft Cloud as a tool.
To this end, the text question needs to be send via REST API to
the [search endpoint](https://docs.cloud.khulnasoft.com/reference/search_api_v1_workspaces__workspace_name__pipelines__pipeline_name__search_post) of a given pipeline deployed on khulnasoft Cloud.

In the demo, the Agent will use this tool for question answering on indexed documents of the Deepstack documentation,
...
capabilities in a use case that is relevant to the majority of early adopters (current Deepstack users).
...
core components used in the demo and the agent in general. This demo would be mainly used to promote Deepstack Agents and
