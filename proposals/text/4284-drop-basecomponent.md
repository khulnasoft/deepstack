...
Deepstack Pipelines are very powerful objects, but they still have a number of unnecessary limitations, by design and by implementation.
...
- `Pipeline` can select among branches, but cannot run such branches in parallel, except for some specific and inconsistent corner cases. For further reference and discussions on the topic, see:
    - https://github.com/khulnasoft/deepstack/pull/2593
    - https://github.com/khulnasoft/deepstack/pull/2981#issuecomment-1207850632
    - https://github.com/khulnasoft/deepstack/issues/2999#issuecomment-1210382151
...
For the detailed explanation behind the design choices and all open questions, see the "Detailed Design" section and the draft implementation here: https://github.com/ZanSara/deepstack-2.0-draft
...
from deepstack.nodes import node

# A Deepstack Node. See below for details about this contract.
...
This query pipeline includes real Reader and Retriever nodes. A new set of primitives and a new DocumentStore has been used for this example: please refer to the draft implementation for details on those: https://github.com/ZanSara/deepstack-2.0-draft
...
from deepstack.data import TextQuery
from deepstack.nodes import node
...
from deepstack.data import TextAnswer, Span
from deepstack import node
...
from deepstack.pipeline import Pipeline
from deepstack.stores import MemoryDocumentStore
from deepstack.data import TextQuery, TextDocument
from deepstack.nodes import RetrieveByBM25, ReadByTransformers
...
This section focuses on the concept rather than the implementation strategy. For a discussion on the implementation, see the draft here: https://github.com/ZanSara/deepstack-2.0-draft
...
- Core functions:
    ...
    - `run(data, parameters, debug)`: the core of the class. Relies on `networkx` for most of the heavy-lifting. Check out the implementation (https://github.com/ZanSara/deepstack-2.0-draft/blob/main/new-deepstack/deepstack/pipeline/pipeline.py) for details: the code is heavily commented on the main loop and on the handling of non-trivial execution paths like branch selection, parallel branch execution, loops handling, multiple input/output and so on. For the effects of `debug`, see the dedicated section below.
...
- Graph building:
    - `add_node(name, node, parameters)`: adds a disconnected node to the graph. It expects Deepstack nodes in the `node` parameter and will fail if they aren't respecting the contract. See below for a more detailed discussion of the Nodes' contract.
...
In this pipeline, several nodes send their input into a single output node. Note that this pipeline has several starting nodes, something that is currently not supported by Deepstack's `Pipeline`.
...
In this pipeline, only one edge will run depending on the decision taken by the `remainder` node. Note that this pipeline has several terminal nodes, something that is currently not supported by Deepstack's `Pipeline`.
...
In this pipeline, all the edges that leave `enumerate` are run by `Pipeline`. Note that this usecase is currently not supported by Deepstack's `Pipeline`.
...
NOTE: the draft implementation supports all of these topologies already. You can find the code for each of these pipelines under https://github.com/ZanSara/new-deepstack-pipeline-draft/tree/main/new-deepstack/tests/integration

## The Node contract

A Deepstack node is any class that abides the following contract:

```python
# This decorator does very little, but is necessary for Pipelines to recognize
# this class as a Deepstack node. Check its implementation for details.
@node
class MyNode:

    def __init__(self, model_name: str: "khulnasoft/a-model-name"):
        """
        Deepstack nodes should have an `__init__` method where they define:
...
When pipelines are loaded from YAML, Pipeline needs to find the classes definition somewhere in the imported modules. Currently, at initialization `Pipeline` looks for classes which is decorated with the `@node` decorator under `deepstack`, however such search can be extended (or narrowed) by setting the `search_nodes_in` init parameter of `Pipeline`. Note that it will try to import any module that is not imported yet.
...
#   File "/home/sara/work/deepstack-2/example.py", line 29, in <module>
#     pipeline.connect('node_a', 'node_a')
#   File "/home/sara/work/deepstack-2/new-deepstack/deepstack/pipeline/pipeline.py", line 224, in connect
#     raise PipelineConnectError(
# deepstack.pipeline._utils.PipelineConnectError: Cannot connect 'node_a' with 'node_a' with an edge named 'intermediate_value': their declared inputs and outputs do not match.
...
#   File "/home/sara/work/deepstack-2/new-deepstack/deepstack/pipeline/pipeline.py", line 224, in connect
#     raise PipelineConnectError(
# deepstack.pipeline._utils.PipelineConnectError: Cannot connect 'node_b' with 'node_a' with an edge named 'output': their declared inputs and outputs do not match.
...
```python
from deepstack.pipelines import Pipeline, save_pipelines, load_pipelines
...
```json
{
    ...
    "nodes": {
        ...
        "my_dense_retriever": {
            ...
            "model_name": "khulnasoft/a-model-name",
        ...
        "my_reader": {
            "class_name": "Reader",
            "model_name": "khulnasoft/another-model-name",
            "top_k": 3
        }
    },
...
This proposal is best thought as part of the design of Deepstack 2.0.

## Rollout process

These changes are going to be release with Deepstack 1.x in a hidden internal package called `deepstack.v2.pipelines`, and won't be promoted straight away.

We will progressively add nodes to this `deepstack.v2` package and build a folder structure under it (`deepstack.v2.nodes`, `deepstack.v2.stores`, ...) version after version, until we believe the content of the package is usable. Documentation will be built in parallel and we will progressively start pushing users towards the 2.0 API.
Power users like dC and other Deepstack experts will be able to test out these changes from the start and provide feedback while still in Deepstack 1.x.

Once we're confident that the v2 version covers all of Deepstack v1.x usecases, Deepstack 2.0 will be released and the packages are going to be switched: the content of `deepstack` will be moved into `deepstack.v1` and deprecated, and the content of `deepstack.v2` will me moved under `deepstack`. A few 2.x versions later, `deepstack.v1` will then be dropped.
