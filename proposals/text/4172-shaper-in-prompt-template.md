...
- Github Issues or Discussion:
  - spike: https://github.com/khulnasoft/deepstack/pull/4061
  - solved issues:
    - https://github.com/khulnasoft/deepstack/issues/3877
    - https://github.com/khulnasoft/deepstack/issues/4053
    - https://github.com/khulnasoft/deepstack/issues/4047

# Summary

In order to make prompt templates more flexible and powerful while at the same time making PromptNode as easy to use as any other node in Deepstack, we want to introduce two modifications to PromptTemplate:
...
A generative QA pipeline would be as easy as this:
    ...
    from deepstack.document_store import InMemoryDocumentStore
...
## Prompt engineering with Deepstack Pipelines
...
- Can the solution you're proposing be implemented as a separate package, outside of Deepstack? No
- Does it teach people more about Deepstack? No, but it makes it easier to use especially for beginners.
...
- What's the cost of migrating existing Deepstack pipelines (is it a breaking change?)? None

It also fosters a bit the nesting of components in Deepstack. Although the whole PromptNode ecosystem already does this (e.g. via PromptModel, PromptTemplate being used by PromptNode), it's still a bit of a new concept. However, I think it's a good one and it's not too hard to understand.
