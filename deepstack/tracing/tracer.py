...
from deepstack import logging

DEEPSTACK_AUTO_TRACE_ENABLED_ENV_VAR = "DEEPSTACK_AUTO_TRACE_ENABLED"
DEEPSTACK_CONTENT_TRACING_ENABLED_ENV_VAR = "DEEPSTACK_CONTENT_TRACING_ENABLED"
...
class Span(abc.ABC):
    ...
    def set_content_tag(self, key: str, value: Any) -> None:
        ...
        - set the environment variable `DEEPSTACK_CONTENT_TRACING_ENABLED` to `true` or
...
def auto_enable_tracing() -> None:
    ...
    This behavior can be disabled by setting the environment variable `DEEPSTACK_AUTO_TRACE_ENABLED` to `false`.
    Note that it will only work correctly if tracing was configured _before_ Deepstack is imported.
    """
    if os.getenv(DEEPSTACK_AUTO_TRACE_ENABLED_ENV_VAR, "true").lower() == "false":
        logger.info(
            "Tracing disabled via environment variable '{env_key}'", env_key=DEEPSTACK_AUTO_TRACE_ENABLED_ENV_VAR
        )
        return
...
def _auto_configured_opentelemetry_tracer() -> Optional[Tracer]:
    ...
    try:
        ...
        with opentelemetry.trace.get_tracer("deepstack").start_as_current_span("deepstack.tracing.auto_enable") as span:
            if isinstance(span, opentelemetry.trace.NonRecordingSpan):
                return None

            from deepstack.tracing.opentelemetry import OpenTelemetryTracer

            return OpenTelemetryTracer(opentelemetry.trace.get_tracer("deepstack"))
    except ImportError:
        pass
...
def _auto_configured_datadog_tracer() -> Optional[Tracer]:
    ...
    try:
        ...
        from deepstack.tracing.datadog import DatadogTracer
