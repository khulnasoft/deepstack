...
from deepstack import ComponentError, Document, component, default_from_dict, default_to_dict, logging
...
class SearchApiWebSearch:
    ...
    Usage example:
    ```python
    from deepstack.components.websearch import SearchApiWebSearch
    ...
    def run(self, query: str) -> Dict[str, Union[List[Document], List[str]]]:
        ...
        headers = {"Authorization": f"Bearer {self.api_key.resolve_value()}", "X-SearchApi-Source": "Deepstack"}
        try:
