...
class TestMemoryDocumentStore(DocumentStoreBaseTests):  # pylint: disable=R0904
    ...
    @patch("deepstack.document_stores.in_memory.document_store.re")
    ...
    def test_bm25_retrieval(self, document_store: InMemoryDocumentStore):
        ...
        docs = [Document(content="Hello world"), Document(content="Deepstack supports multiple languages")]
        document_store.write_documents(docs)
        results = document_store.bm25_retrieval(query="What languages?", top_k=1)
        assert len(results) == 1
        assert results[0].content == "Deepstack supports multiple languages"
    ...
    def test_bm25_retrieval_with_different_top_k(self, document_store: InMemoryDocumentStore):
        ...
        docs = [
            ...
            Document(content="Deepstack supports multiple languages"),
    ...
    def test_bm25_retrieval_with_non_scaled_BM25Okapi(self):
        ...
        docs = [
            Document(
                content="""Use pip to install a basic version of Deepstack's latest release: pip install
                farm-deepstack. All the core Deepstack components live in the deepstack repo. But there's also the
                deepstack-extras repo which contains components that are not as widely used, and you need to
                install them separately."""
            ),
            Document(
                content="""Use pip to install a basic version of Deepstack's latest release: pip install
                farm-deepstack[inference]. All the core Deepstack components live in the deepstack repo. But there's
                also the deepstack-extras repo which contains components that are not as widely used, and you need
                to install them separately."""
            ),
            Document(
                content="""Use pip to install only the Deepstack 2.0 code: pip install deepstack-ai. The deepstack-ai
        ...
        results1 = document_store.bm25_retrieval(query="Deepstack installation", top_k=10, scale_score=False)
        assert len(results1) == 3
        assert all(res.score < 0.0 for res in results1)

        results2 = document_store.bm25_retrieval(query="Deepstack installation", top_k=10, scale_score=True)
    ...
    def test_embedding_retrieval(self):
        ...
        docs = [
            ...
            Document(content="Deepstack supports multiple languages", embedding=[1.0, 1.0, 1.0, 1.0]),
        ]
    ...
    def test_embedding_retrieval_documents_different_embedding_sizes(self):
        ...
        docs = [
            Document(content="Hello world", embedding=[0.1, 0.2, 0.3, 0.4]),
            Document(content="Deepstack supports multiple languages", embedding=[1.0, 1.0]),
        ]
        docstore.write_documents(docs)
