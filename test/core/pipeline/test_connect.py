...
from deepstack.core.errors import PipelineConnectError
from deepstack.core.pipeline import Pipeline
from deepstack.core.pipeline.pipeline import parse_connect_string
...
def test_connect():
    ...
    assert comp1.__deepstack_output__.value.receivers == ["comp2"]
    assert comp2.__deepstack_input__.value.senders == ["comp1"]
...
def test_multiple_outputs_to_variadic_input():
    ...
    assert comp1.__deepstack_output__.value.receivers == ["comp3"]
    assert comp2.__deepstack_output__.value.receivers == ["comp3"]
    assert comp3.__deepstack_input__.value.senders == ["comp1", "comp2"]
