...
from deepstack.core.component.types import InputSocket, OutputSocket
from deepstack.core.errors import PipelineDrawingError, PipelineError, PipelineMaxLoops, PipelineRuntimeError
from deepstack.core.pipeline import Pipeline, PredefinedPipeline
from deepstack.core.serialization import DeserializationCallbacks
from deepstack.document_stores.in_memory import InMemoryDocumentStore
from deepstack.testing.factory import component_class
from deepstack.testing.sample_components import AddFixedValue, Double, Greet
...
def test_run_with_greedy_variadic_after_component_with_default_input_simple(spying_tracer):
    ...
    assert spying_tracer.spans[0].operation_name == "deepstack.pipeline.run"
    assert spying_tracer.spans[1].operation_name == "deepstack.component.run"
    assert spying_tracer.spans[1].tags["deepstack.component.name"] == "retriever"
    assert spying_tracer.spans[2].operation_name == "deepstack.component.run"
    assert spying_tracer.spans[2].tags["deepstack.component.name"] == "multiplexer"
    assert spying_tracer.spans[3].operation_name == "deepstack.component.run"
    assert spying_tracer.spans[3].tags["deepstack.component.name"] == "prompt_builder"
...
def test_pipeline_resolution_duplicate_input_names_across_components():
    ...
    result = pipe.run(data={"what": "Deepstack", "who": "world"})
    assert result == {"hello2": {"output": "Hello Hello world Deepstack! Deepstack!"}}

    resolved, _ = pipe._prepare_component_input_data(data={"what": "Deepstack", "who": "world"})

    # why does hello2 have only one input? Because who of hello2 is inserted from hello.output
    assert resolved == {"hello": {"what": "Deepstack", "who": "world"}, "hello2": {"what": "Deepstack"}}
...
@patch("deepstack.core.pipeline.pipeline._to_mermaid_image")
@patch("deepstack.core.pipeline.pipeline.is_in_jupyter")
...
def test_add_component_to_different_pipelines():
    ...
    assert some_component.__deepstack_added_to_pipeline__ is None
    first_pipe.add_component("some", some_component)
    assert some_component.__deepstack_added_to_pipeline__ is first_pipe
...
def test_to_dict():
    ...
    expected = {
        ...
        "components": {
            "add_two": {
                "type": "deepstack.testing.sample_components.add_value.AddFixedValue",
...
def test_from_template(monkeypatch):
    ...
    with patch("deepstack_integrations.document_stores.chroma.document_store.ChromaDocumentStore"):
...
def test_correct_execution_order_of_components_with_only_defaults(spying_tracer):
    ...
    assert spying_tracer.spans[1].tags["deepstack.component.name"] == "retriever"
    assert spying_tracer.spans[2].operation_name == "deepstack.component.run"
    assert spying_tracer.spans[2].tags["deepstack.component.name"] == "prompt_builder"
