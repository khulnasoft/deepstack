...
from deepstack.core.component import Component, InputSocket, OutputSocket, component
from deepstack.core.component.component import _hook_component_init
from deepstack.core.component.types import Variadic
from deepstack.core.errors import ComponentError
...
def test_set_input_types():
    ...
    comp = MockComponent()
    assert comp.__deepstack_input__._sockets_dict == {"value": InputSocket("value", Any)}
...
def test_set_output_types():
    ...
    comp = MockComponent()
    assert comp.__deepstack_output__._sockets_dict == {"value": OutputSocket("value", int)}
...
def test_input_has_default_value():
    ...
    comp = MockComponent()
    assert comp.__deepstack_input__._sockets_dict["value"].default_value == 42
    assert not comp.__deepstack_input__._sockets_dict["value"].is_mandatory
...
def test_keyword_only_args():
    ...
    comp = MockComponent()
    component_inputs = {name: {"type": socket.type} for name, socket in comp.__deepstack_input__._sockets_dict.items()}
...
def test_is_greedy_default_with_variadic_input():
    ...
    assert not MockComponent.__deepstack_is_greedy__
    assert not MockComponent().__deepstack_is_greedy__
...
def test_is_greedy_flag_with_variadic_input():
    ...
    assert MockComponent.__deepstack_is_greedy__
    assert MockComponent().__deepstack_is_greedy__
