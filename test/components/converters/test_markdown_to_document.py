...
class TestMarkdownToDocument:
    ...
    def test_run(self, test_files_path):
        ...
        assert len(docs) == 1
        for doc in docs:
            assert "What to build with Deepstack" in doc.content
            assert "# git clone https://github.com/khulnasoft/deepstack.git" in doc.content
    ...
    def test_run_calls_normalize_metadata(self, test_files_path):
        ...
        with patch("deepstack.components.converters.markdown.normalize_metadata") as normalize_metadata, patch(
            "deepstack.components.converters.markdown.MarkdownIt"
        ):
    ...
    def test_run_with_meta(self, test_files_path):
        ...
        with patch("deepstack.components.converters.markdown.MarkdownIt"):
