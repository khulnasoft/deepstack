...
from deepstack.dataclasses import ByteStream
...
class TestHTMLToDocument:
    def test_run(self, test_files_path):
        ...
        """
        sources = [test_files_path / "html" / "what_is_deepstack.html"]
        converter = HTMLToDocument()
        results = converter.run(sources=sources, meta={"test": "TEST"})
        docs = results["documents"]
        assert len(docs) == 1
        assert "Deepstack" in docs[0].content
    ...
    def test_run_doc_metadata(self, test_files_path):
        ...
        converter = HTMLToDocument()
        sources = [test_files_path / "html" / "what_is_deepstack.html"]
        metadata = [{"file_name": "what_is_deepstack.html"}]
        results = converter.run(sources=sources, meta=metadata)
        docs = results["documents"]

        assert len(docs) == 1
        assert "Deepstack" in docs[0].content
        assert docs[0].meta["file_name"] == "what_is_deepstack.html"
    ...
    def test_incorrect_meta(self, test_files_path):
        ...
        converter = HTMLToDocument()
        sources = [test_files_path / "html" / "what_is_deepstack.html"]
        metadata = [{"file_name": "what_is_deepstack.html"}, {"file_name": "deepstack.html"}]
    ...
    def test_run_bytestream_metadata(self, test_files_path):
        ...
        """
        converter = HTMLToDocument()
        with open(test_files_path / "html" / "what_is_deepstack.html", "rb") as file:
    ...
    def test_run_bytestream_and_doc_metadata(self, test_files_path):
        ...
        assert len(docs) == 1
        assert "Deepstack" in docs[0].content
        assert docs[0].meta == {"file_name": "what_is_deepstack.html", "content_type": "text/html", "url": "test_url"}
    ...
    def test_run_bytestream_doc_overlapping_metadata(self, test_files_path):
        ...
        metadata = [{"file_name": "what_is_deepstack.html", "url": "test_url_new"}]
        results = converter.run(sources=[stream], meta=metadata)
        docs = results["documents"]
        ...
        assert "Deepstack" in docs[0].content
        assert docs[0].meta == {
            "file_name": "what_is_deepstack.html",
    ...
    def test_mixed_sources_run(self, test_files_path):
        ...
        """
        sources = [
            test_files_path / "html" / "what_is_deepstack.html",
            str((test_files_path / "html" / "what_is_deepstack.html").absolute()),
        ]
        with open(test_files_path / "html" / "what_is_deepstack.html", "rb") as f:
            byte_stream = f.read()
        ...
        assert len(docs) == 3
        for doc in docs:
            assert "Deepstack" in doc.content
