...
class TestAzureOCRDocumentConverter:
    ...
    @patch("deepstack.utils.auth.EnvVarSecret.resolve_value")
    def test_to_dict(self, mock_resolve_value):
        ...
        assert data == {
            "type": "deepstack.components.converters.azure.AzureOCRDocumentConverter",
            "init_parameters": {
    ...
    def test_with_image_file(self, test_files_path):
        ...
        output = component.run(sources=[test_files_path / "images" / "deepstack-logo.png"])
        documents = output["documents"]
        assert len(documents) == 1
        assert "deepstack" in documents[0].content
        assert "by khulnasoft" in documents[0].content
