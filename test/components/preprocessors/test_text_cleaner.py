from deepstack.components.preprocessors import TextCleaner
...
def test_run_with_regex():
    ...
    result = cleaner.run(texts=["Open123 Source", "DeepstackAI"])
    assert len(result) == 1
    assert result["texts"] == ["Open Source", "DeepstackAI"]


def test_run_with_multiple_regexps():
    cleaner = TextCleaner(remove_regexps=[r"\d+", r"[^\w\s]"])
    result = cleaner.run(texts=["Open123! Source", "Deepstack.AI"])
    assert len(result) == 1
    assert result["texts"] == ["Open Source", "DeepstackAI"]
...
def test_run_with_convert_to_lowercase():
    ...
    result = cleaner.run(texts=["Open123! Source", "Deepstack.AI"])
    assert len(result) == 1
    assert result["texts"] == ["open123! source", "deepstack.ai"]
...
def test_run_with_remove_punctuation():
    ...
    result = cleaner.run(texts=["Open123! Source", "Deepstack.AI"])
    assert len(result) == 1
    assert result["texts"] == ["Open123 Source", "DeepstackAI"]
...
def test_run_with_remove_numbers():
    ...
    result = cleaner.run(texts=["Open123! Source", "Deepstack.AI"])
    assert len(result) == 1
    assert result["texts"] == ["Open! Source", "Deepstack.AI"]
...
def test_run_with_multiple_parameters():
    ...
    result = cleaner.run(texts=["Open%123. !$Source", "Deepstack.AI##"])
    assert len(result) == 1
    assert result["texts"] == ["open source", "deepstackai"]
