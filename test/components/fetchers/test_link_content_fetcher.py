...
from deepstack.components.fetchers.link_content import (
    LinkContentFetcher,
...
HTML_URL = "https://docs.deepstack.khulnasoft.com/docs"
TEXT_URL = "https://raw.githubusercontent.com/khulnasoft/deepstack/main/README.md"
PDF_URL = "https://raw.githubusercontent.com/khulnasoft/deepstack/b5987a6d8d0714eb2f3011183ab40093d2e4a41a/e2e/samples/pipelines/sample_pdf_1.pdf"


@pytest.fixture
def mock_get_link_text_content():
    with patch("deepstack.components.fetchers.link_content.requests") as mock_run:
...
class TestLinkContentFetcher:
    ...
    def test_link_content_fetcher_html(self):
        ...
        first_stream = streams[0]
        assert "Deepstack" in first_stream.data.decode("utf-8")
        assert first_stream.meta["content_type"] == "text/html"
    ...
    def test_link_content_fetcher_multiple_different_content_types(self):
        ...
        for stream in streams:
            ...
            if stream.meta["content_type"] == "text/html":
                assert "Deepstack" in stream.data.decode("utf-8")
    ...
    def test_link_content_fetcher_multiple_html_streams(self):
        ...
        for stream in streams:
            assert stream.meta["content_type"] in ("text/html", "application/pdf", "application/octet-stream")
            if stream.meta["content_type"] == "text/html":
                assert "Deepstack" in stream.data.decode("utf-8") or "Google" in stream.data.decode("utf-8")
            elif stream.meta["content_type"] == "application/pdf":
                assert len(stream.data) > 0
