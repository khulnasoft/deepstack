...
from deepstack import Document, ExtractedAnswer
from deepstack.components.readers import ExtractiveReader
from deepstack.utils import Secret
from deepstack.utils.device import ComponentDevice, DeviceMap
...
def mock_tokenizer():
    ...
    with patch("deepstack.components.readers.extractive.AutoTokenizer.from_pretrained") as tokenizer:
...
def mock_reader(mock_tokenizer):
    ...
    with patch("deepstack.components.readers.extractive.AutoModelForQuestionAnswering.from_pretrained") as model:
...
def test_to_dict(initialized_token: Secret):
    ...
    assert data == {
        "type": "deepstack.components.readers.extractive.ExtractiveReader",
        "init_parameters": {
            "model": "my-model",
            "device": None,
...
@patch("deepstack.components.readers.extractive.AutoTokenizer.from_pretrained")
@patch("deepstack.components.readers.extractive.AutoModelForQuestionAnswering.from_pretrained")
def test_warm_up_use_hf_token(mocked_automodel, mocked_autotokenizer, initialized_token: Secret):
    reader = ExtractiveReader("khulnasoft/roberta-base-squad2", device=ComponentDevice.from_str("cpu"))
    ...
    mocked_automodel.assert_called_once_with("khulnasoft/roberta-base-squad2", token="secret-token", device_map="cpu")
    mocked_autotokenizer.assert_called_once_with("khulnasoft/roberta-base-squad2", token="secret-token")
...
def test_device_map_auto(mocked_automodel, _mocked_autotokenizer, monkeypatch):
    ...
    reader = ExtractiveReader("khulnasoft/roberta-base-squad2", model_kwargs={"device_map": "auto"})
    ...
    mocked_automodel.assert_called_once_with("khulnasoft/roberta-base-squad2", token=None, device_map="auto")
...
def test_device_map_str(mocked_automodel, _mocked_autotokenizer, monkeypatch):
    monkeypatch.delenv("HF_API_TOKEN", raising=False)
    reader = ExtractiveReader("khulnasoft/roberta-base-squad2", model_kwargs={"device_map": "cpu:0"})
    ...
    mocked_automodel.assert_called_once_with("khulnasoft/roberta-base-squad2", token=None, device_map="cpu:0")
...
def test_device_map_dict(mocked_automodel, _mocked_autotokenizer, monkeypatch):
    ...
    reader = ExtractiveReader(
        "khulnasoft/roberta-base-squad2", model_kwargs={"device_map": {"layer_1": 1, "classifier": "cpu"}}
    )
    ...
    mocked_automodel.assert_called_once_with(
        "khulnasoft/roberta-base-squad2", token=None, device_map={"layer_1": 1, "classifier": "cpu"}
    )
...
@pytest.mark.integration
def test_roberta():
    reader = ExtractiveReader("khulnasoft/tinyroberta-squad2")
    reader.warm_up()
    answers = reader.run(example_queries[0], example_documents[0], top_k=2)[
        "answers"
...
def test_matches_hf_pipeline():
    reader = ExtractiveReader(
        "khulnasoft/tinyroberta-squad2", device=ComponentDevice.from_str("cpu"), overlap_threshold=None
    )
    reader.warm_up()
