...
from deepstack.components.embedders import HuggingFaceAPITextEmbedder
from deepstack.utils.auth import Secret
from deepstack.utils.hf import HFEmbeddingAPIType


@pytest.fixture
def mock_check_valid_model():
    with patch(
        "deepstack.components.embedders.hugging_face_api_text_embedder.check_valid_model", MagicMock(return_value=None)
    ) as mock:
        yield mock
...
class TestHuggingFaceAPITextEmbedder:
    ...
    def test_to_dict(self, mock_check_valid_model):
        ...
        assert data == {
            "type": "deepstack.components.embedders.hugging_face_api_text_embedder.HuggingFaceAPITextEmbedder",
            "init_parameters": {
                "api_type": HFEmbeddingAPIType.SERVERLESS_INFERENCE_API,
                "api_params": {"model": "BAAI/bge-small-en-v1.5"},
