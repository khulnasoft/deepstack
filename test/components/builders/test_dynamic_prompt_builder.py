...
from deepstack import Document, Pipeline, component
from deepstack.components.builders import DynamicPromptBuilder
...
class TestDynamicPromptBuilder:
    def test_initialization(self):
        ...
        assert set(builder.__deepstack_input__._sockets_dict.keys()) == expected_keys

        # response is always prompt regardless of chat mode
        assert set(builder.__deepstack_output__._sockets_dict.keys()) == {"prompt"}

        # prompt_source is a list of ChatMessage or a string
        assert builder.__deepstack_input__._sockets_dict["prompt_source"].type == str

        # output is always prompt, but the type is different depending on the chat mode
        assert builder.__deepstack_output__._sockets_dict["prompt"].type == str
